...
class PortImplementationArchitectureTest {
    ...
    private val adapterPackages = arrayOf(
        "..resultingestionservice.adapter.inbound..",
        "..resultingestionservice.adapter.outbound..",
        "..resultingestionservice.adapter.in..",
        "..resultingestionservice.adapter.out.."
    )
    ...
    fun `interfaces ending with Port should live in port packages`() {
        val rule = classes()
            ...
            .should().resideInAnyPackage(
                "..resultingestionservice.domain.port..",
                "..resultingestionservice.application.port.."
            )
    ...
    fun `implementations of port interfaces should end with Adapter`() {
        val rule = classes()
            .that().implement(resideInAnyPackage("..resultingestionservice..port.."))
